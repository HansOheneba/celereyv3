...
export default function WealthScanResultsPage() {
  ...
  useEffect(() => {
    ...
    if (storedResults) {
      try {
        const parsed = JSON.parse(storedResults) as WealthHealthResults;
        if (parsed.email) setResults(parsed);
        else router.push("/wealthscan");
      } catch {
        router.push("/wealthscan");
      }
    } else {
      router.push("/wealthscan");
    }
  ...
  return (
    <div className="min-h-screen max-w-360 mx-auto px-6">
      {/* Hero — matches homepage split layout */}
      <section className=" py-16 sm:py-20">
        <div className="mx-auto grid w-full max-w-7xl px-6 grid-cols-1 items-center gap-10 md:grid-cols-2 md:gap-12">
          {/* Left: text + score */}
          <motion.div
          ...
          >
            ...
            <div className="flex  items-end gap-3 mb-4">
              <span className="font-cirka font-thin text-[96px] sm:text-[120px] text-primary leading-none">
                {results.score}
              </span>
              <span className="text-2xl text-muted-foreground font-light pb-4">
                /100
              </span>
            </div>
            ...
            <div className="flex flex-wrap gap-3 mb-8">
              {results.topPillars.slice(0, 2).map((p) => (
                ...
                >
                  <span className="h-2 w-2 rounded-full bg-[#CDE6AF] border border-green-300" />
                  {p}
                </span>
              ))}
              {results.bottomPillars.slice(0, 2).map((p) => (
                <span
                  ...
                  className="flex items-center gap-2 text-sm text-foreground"
                >
                  <span className="h-2 w-2 rounded-full bg-rose-200 border border-rose-300" />
                  {p}
                </span>
              ))}
            </div>
      ...
      {/* 02 Chart */}
      <div className="w-full px-6 sm:px-12 mb-10 rounded-3xl lg:px-20 py-14 bg-white">
        <motion.div
          initial={{ opacity: 0, y: 14 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
      ...
      {/* Interlude */}
      <div className="relative w-full h-[42vh] sm:h-[50vh] rounded-3xl overflow-hidden">
        <Image
      ...
      {results.recommendations.length > 0 && (
        <div className="relative w-full overflow-hidden bg-primary px-6 sm:px-12 lg:px-20 py-14 rounded-3xl mb-8">
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              {results.recommendations.map((rec, i) => (
                <div
                  key={i}
                  className="flex gap-4 rounded-2xl bg-white/10 border border-white/10 p-5"
                >
